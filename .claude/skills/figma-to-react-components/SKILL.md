...
3. **Stories location** — Where Storybook stories live. **Default to co-locating** the story next to the component (`MyComponent/MyComponent.stories.tsx`) — this is the recommended pattern for component libraries and what most mature design systems use. Only deviate if the project clearly mirrors a separate `stories/` tree for component stories. Non-component stories (foundations, tokens, intros, guidelines) belong in a dedicated docs folder such as `src/docs/` and are written as `.mdx`.
...
Cache these findings for the session. If unsure about any setting, ask the user before generating code. Ask the user if they're using a icon library dependency, if so, use that instead of local svg files (for icons at least). Do not waste tokens on generating SVG's if they exist or using a library.
...
Each component lives in its own folder, co-locating all related artifacts:

```
<components-root>/<ComponentName>/
├── <ComponentName>.tsx              # component
├── <ComponentName>.module.css       # styles (CSS Modules) — consume var(--…) tokens only
├── <ComponentName>.stories.tsx      # Storybook story
├── <ComponentName>.test.tsx         # Vitest + @testing-library/react
└── index.ts                         # re-export
```

Then add to the library's public API in the project's main entry file (e.g. `src/index.ts`):

```ts
export * from './<components-root>/<ComponentName>';
```

Adjust file extensions (`.scss`, `.css`, `.tsx`, `.ts`) and the components root path to match the project's discovered conventions from the Project Configuration Discovery step.
...
**Location:** Co-locate the story file with the component by default (`src/components/<level>/MyComponent/MyComponent.stories.tsx`). The story moves with the component on rename/delete, contributors see all artifacts (tsx, css, test, story) in one place, and refactors don't desync. Only place stories under a separate `stories/` tree if the project already follows that convention.

Non-component documentation (foundations, tokens, typography, spacing, intros, changelogs) is **not** a component story — author it as `.mdx` in a dedicated docs folder (e.g., `src/docs/`) so the Storybook sidebar has a clean `Foundations/*` section separate from `Atoms/*`, `Molecules/*`, `Organisms/*`.
...
4. **Storybook Story** (`[ComponentName].stories.tsx`, **co-located** with the component)
   - Interactive examples with React Aria props
   - All variants demonstrated
   - For non-component documentation (foundations, tokens, intros), author `.mdx` files in the project's docs folder (e.g., `src/docs/`) instead of co-locating
